...
export class MeteoraOnChainProvider {
     ...
     */
    public async buildClaimFeesTransactions(
        ...
        return await this.rpcProvider.execute(async () => {
            ...
            if (!targetPosition) {
                throw new Error(`[MeteoraOnChain] Position ${params.positionPubkey.toBase58()} not found for user ${params.userWallet.toBase58()}`);
            }
    ...
    public async buildRemoveLiquidityTransactions(params: RemoveLiquidityParams): Promise<Transaction[]> {
        ...
        logger.info(`[MeteoraOnChain] Building RemoveLiquidity Txs for position ${params.positionPubkey.toBase58()} (Bins: ${params.lowerBinId} to ${params.upperBinId})`);
