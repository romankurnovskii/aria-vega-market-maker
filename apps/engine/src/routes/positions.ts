...
import { positionGuard } from '../services/position-guard.js';
...
export function handlePositionsRouter(
  ...
  router.post('/:positionId/actions', async (req, res) => {
    ...
    try {
      ...
      await Promise.race([
        timeoutPromise,
        positionGuard.runExclusive(positionId, async () => {
              ...
              if (openRecord.status === 'failed') {
                if (targetOrch && !targetOrch.assignmentId.startsWith('adhoc_')) {
                  ...
                  if (oldAssignment) {
                    ...
                    logger.info(
                      `[HTTP Server] Assignment ${oldAssignment.id} set to pending_open. Will retry on next tick.`
                    );
                  }
                }
      ...
      if (errorMsg === 'TIMEOUT') {
        res.status(504).json({ error: 'Request timed out. The operation may still be executing in the background.' });
